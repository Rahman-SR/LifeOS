...
export function DeleteNoteConfirmation({ loading, name, onCancel, onConfirm, visible }: { loading: boolean; name: string; onCancel: () => void; onConfirm: () => void; visible: boolean }) {
  const { colors } = useAppTheme();
  return (
    <Modal animationType="fade" onRequestClose={onCancel} statusBarTranslucent transparent visible={visible}>
      <View style={styles.modal}>
        <Pressable accessibilityLabel="Cancel note deletion" accessibilityRole="button" onPress={onCancel} style={StyleSheet.absoluteFill}>
        ...
        >
          <View style={styles.header}>
            ...
            <AppText style={styles.title} variant="heading3">Delete note?</AppText>
...
const styles = StyleSheet.create({
  ...
  dialog: { borderRadius: radii.large, borderWidth: sizing.border, gap: spacing.md, marginHorizontal: spacing.lg, padding: spacing.lg },
