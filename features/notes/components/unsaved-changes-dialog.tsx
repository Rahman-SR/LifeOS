...
export function UnsavedChangesDialog({ onDiscard, onKeepEditing, visible }: { onDiscard: () => void; onKeepEditing: () => void; visible: boolean }) {
  ...
  return (
    <Modal animationType="fade" onRequestClose={onKeepEditing} statusBarTranslucent transparent visible={visible}>
      <View style={styles.modal}>
        <Pressable accessibilityLabel="Keep editing note" accessibilityRole="button" onPress={onKeepEditing} style={StyleSheet.absoluteFill}>
        ...
        <SafeAreaView accessibilityViewIsModal edges={['left', 'right']} style={[styles.dialog, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <View style={styles.header}>
            <AlertTriangle color={colors.warning} size={sizing.icon} />
            <AppText variant="heading3">Discard unsaved changes?</AppText>
          </View>
          <AppText tone="secondary">Your note changes have not been saved.</AppText>
